...
[Ссылка для скачивания](https://drive.google.com/drive/folders/1xBvawcHeOYW_kbzc2Y9lJvGF8viqKA0t?usp=sharing)
## 1. Подготовка директорий и файлов

### Директория `RAG2`
Скачать в директорию `RAG2` следующие файлы:
- `faiss_index.idx`
- `texts_metas.pkl`

А также папку `data`, внутри которой должны быть 2 папки:
...
### Директория `sqlcoder_finetuned_2`
Скачать в директорию `sqlcoder_finetuned_2` файлы:
- `adapter_config.json`
...
### Директория `static`
Скачать в директорию `static` файл:
- `static.css`

### Директория `templates`
Скачать в директорию `templates` файлы:
- `results.html`
- `results1.html`
- `results2.html`
...
Объединить все 4 директории в одну корневую директорию с файлами


## 3. Запуск приложения

Запустить `main.py` по инструкции:
